...
class RVIin(bt.Indicator):
    ...
    def next(self):
        self.lines.NUM[0] = (self.data.close - self.data.open + 2*(self.data.close[-1] - self.data.open[-1]) + 2*(self.data.close[-2] - self.data.open[-2]) + self.data.close[-3] - self.data.open[-3])/6 
        ...
        if avDEM ==0:
            ...
            self.lines.RVI[0] = (avNUM/self.maperiod)/(avDEM/self.maperiod)
        try:
