...
def chkhold():
    ...
    if NumPos > 0:
        ...
        openprice = order.loc[order['create_time'] == order['create_time'].max()].price.values
    ...
    while True:
        ...
        if query.loc[query.index[query['code'] == 'HK.' + str(code)].min()].order_status == 'FILLED_ALL':
            ...
            if ret == RET_OK:
                ...
                while ret != RET_OK:
                    ret,order = trd_ctx.order_list_query(trd_env = TrdEnv.SIMULATE)
            print(trd_ctx.modify_order(ModifyOrderOp.CANCEL,str(order.loc[order.index['code' == 'HK.' + str(code)].max()].order_id.values),price = close, qty = size*hand,trd_env = TrdEnv.SIMULATE))       
            break 
