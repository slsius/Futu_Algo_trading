...
def chkhold():
    ...
    if NumPos > 0:
        ...
        openprice =  order.loc[order.index[order['code'] == 'HK.' + str(code)].max()].price
    trd_ctx.close()
...
def buy(close):
    ...
    while True:
        ...
        if ret == RET_OK:
            ...
            while ret != RET_OK:
                ret, query = trd_ctx.order_list_query(trd_env = TrdEnv.SIMULATE)
        if query.loc[query.index[qurty['code'] == 'HK.' + str(code)].min()].order_status == 'FILLED_ALL':
            ...
            if ret == RET_OK:
                ...
                while ret != RET_OK:
                    ret,order = trd_ctx.order_list_query(trd_env = TrdEnv.SIMULATE)
            print(trd_ctx.modify_order(ModifyOrderOp.CANCEL,str(order.loc[order.index['code' == 'HK.' + str(code)].max()].order_id),price = close, qty = size*hand,trd_env = TrdEnv.SIMULATE))       
            break 
