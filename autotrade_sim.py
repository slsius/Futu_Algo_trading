...
def buy(close):
    ...
    if len(orderinfo) > 0: #check is it ordered within 2mins
        if orderinfo.iloc[0].trd_side == 'BUY' and rderinfo.iloc[0].order_status == 'FILLED_ALL':
            ...
            if diff.total_seconds()/60 < 6:
                ...
                return 0
        if orderinfo.iloc[-1].trd_side == 'BUY' and rderinfo.iloc[-1].order_status == 'FILLED_ALL':
